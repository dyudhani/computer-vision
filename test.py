...
# def display_pixel_info(pixel_data, title):
#     st.write(title)
#     df = pd.DataFrame(pixel_data, columns=["Pixel", "Red", "Green", "Blue"])
#     st.table(df)
...
#     if image is not None and change_resolution:
#         image = resize_image(image, (50, 50))
#         st.image(convert_to_rgb(image), caption='Gambar yang Diunggah (50x50)')
#     elif image is not None:
#         st.image(convert_to_rgb(image), caption='Gambar yang Diunggah')

#     # Tampilkan warna pixel saat mengklik pada gambar
#     st.write("Klik pada gambar untuk melihat warna pixel:")
#     clicked = st.empty()

#     if st.button("Reset"):
#         clicked.empty()

#     if st.image_clicked(clicked=clicked):
#         x, y = st.image_coordinates(clicked=clicked)
#         show_pixel_color(image, x, y)
